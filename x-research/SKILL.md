...
- **Do not strip the wrapper's safety flags.** grok auto-loads all user skills from `~/.claude/skills` (it's Claude Code-compatible), including this one — without `--disallowed-tools Shell,...` and `--no-subagents` it will recursively invoke this very skill and fan out runaway grok processes. (Grok's real tool IDs are `Shell`, `Task`, `Write`, `StrReplace` — NOT the `run_terminal_cmd`-style names in grok's own docs.) If a run ever hangs >5 min with no output, check `ps aux | grep grok` for a process explosion and `pkill -f "grok --cwd"`.
